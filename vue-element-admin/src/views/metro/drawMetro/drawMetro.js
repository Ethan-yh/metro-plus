...
// 起点站UI
var startUI = new Q.ImageUI(require('./images/start.png'));
...
//终点站UI
var endUI = new Q.ImageUI(require('./images/end.png'));
...
function drawMetro(canvas, metroData) {
    ...
    // const path = lineNodes['l11'].image;
    // const pos = path.getLocation(1010)
    // console.log(path)
    // console.log('pos:',pos.rotate)
    // console.log('pathboundscx:',path.bounds.cx, 'pathboundscy:',path.bounds.cy)
    // var node = graph.createNode('train', pos.x, pos.y)
    return graph;
}


// 绘制进站图
function drawPassEntry(entryData) {

    passEntryNodes = {}
    for (let i in entryData) {
        const record = entryData[i];
        const stationNode = getStationNode(record.station_id);
        const lineNode = lineNodes[stationNode.data.lines[0].id].data;
        if (!stationNode) {
            continue;
        }
        var node = graph.createNode(null, stationNode.data.x, stationNode.data.y);
        node.tooltip = '' + record.entry;
        let size;
        size = Math.sqrt(record.entry * 10);
        var circle = Q.Shapes.getShape(Q.Consts.SHAPE_CIRCLE, size, size);
        node.image = circle;
        node.zIndex = 15;
        node.movable = false;
        node.setStyle(Q.Styles.SHAPE_FILL_COLOR, lineNode.color);
        node.setStyle(Q.Styles.SHAPE_STROKE_STYLE, lineNode.color);
        node.setStyle(Q.Styles.ALPHA, 0.5)

        passEntryNodes[record.station_id] = node;
    }
}

function clearPassEntry() {
    for (let i in passEntryNodes) {
        graph.removeElement(passEntryNodes[i]);
    }
    passEntryNodes = {};
}
...
function createStation(station) {
    ...
    if (isTransferStation(station)) {
        node.image = require("./images/transfer.png");
        node.size = { width: 16 }
    } else {
        node.image = circle;
    }
...
var endStation = null;
function getOD() {
    return { o_sid: startStation.data.id, d_sid: endStation.data.id }
}
// module.exports = drawMetro;

export default { drawMetro, drawPassEntry, clearPassEntry, getOD, drawPath };
